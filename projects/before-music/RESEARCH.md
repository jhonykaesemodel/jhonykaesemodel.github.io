...
- **Boundary:** The decoded samples and derived frequency activity are calculated. Space, color, displacement, virtual sources, cochlear geometry, and neural geometry are explanatory visual amplifications—not a room-acoustics or brain simulation.

## Claim ledger

| Claim used in the experience | Source | Visual decision |
| --- | --- | --- |
| Sound in air propagates as alternating longitudinal compression and rarefaction while local parcels oscillate about equilibrium. | [OpenStax, Sound Waves](https://openstax.org/books/university-physics-volume-1/pages/17-1-sound-waves) | A stable particle cloud shifts locally and collectively; a faint continuous field keeps the pressure sign legible. Random jitter evokes a restless atmosphere but is not a molecular or thermodynamic simulation. |
...
- Air samples a 32 ms stereo PCM window synchronized to the audio clock. The two channels are shown as idealized virtual plane-wave sources and blended across the field. A signed nonlinear display curve magnifies quiet pressure while preserving zero crossings and direction. Up to 7,200 GPU particles remain near fixed equilibrium positions; their small randomized jitter is expressive, while their coherent displacement and color come from the decoded signal.
